...
//
// זה §6.1.ג.1 (חלונות) ו-ג.2 (קצב 4-3-4-3) גם יחד, ושניהם **נגזרים** ולא
// מקודדים. שני האילוצים היחידים:
//   · רשתות פנויות בסלוט
//   · g(S) + g(S+1) ≤ ⌊N/2⌋ — כי שני סלוטים רצופים דורשים 2·(g(S)+g(S+1))
//     קבוצות **שונות** (אין רצף), ויש רק N. זה, ורק זה, מייצר את 4-3-4-3.
//
// הליגות נארזות קדימה לפי הסדר. ל-15/15/6 עם 4 רשתות ו-16 סלוטים זה מוציא
// בדיוק את §6.1.ג.1 — שואו רשת 1 סלוטים 1–8 · ליגה א׳ 1–9 · ליגה ב׳ 9–16 —
// כי אחרי שואו וליגה א׳ אין תא פנוי לפני סלוט 9. לא נכתב שום מספר.
export function planSlotCounts(ctx) {
  ...
  // ליגה עם רשת קבועה (שואו, החלטה 4) חייבת לתכנן ראשונה: היא היחידה שלא
  // יכולה להסתפק ב"איזו רשת שהיא", ואם ליגה גמישה תיקח את הרשת שלה קודם —
  // התוכנית תהיה לא ישימה.
  const plan = {}, overflow = {};
